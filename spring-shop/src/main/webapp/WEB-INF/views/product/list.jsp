...
<body>
				...
				<form id="form-products" method="get" action ="list">
					...
					<div class="mb-3 d-flex justify-content-between">
						<select class="form-select w-25" name="rows" onchange="changeRows()">
							...
							<option value="10" ${param.rows or param.rows eq 10? 'selected' : '' }> 10개씩보기</option>
							<option value="15" ${param.rows eq 20? 'selected' : '' }> 20개씩보기</option>
							<option value="20" ${param.rows eq 50? 'selected' : '' }> 50개씩보기</option>
						</select>
						<div>
							<div class="form-check form-check-inline">
								...
								name="sort" 
								value="date"
								${param.sort or param.sort eq 'date'? 'checked' : '' }
								onchange="changeSort()"/>
							...
							<div class="form-check form-check-inline">
								...
								name="sort" 
								value="name"
								${param.sort or param.sort eq 'name'? 'checked' : '' }
								onchange="changeSort()"/>
							...
							<div class="form-check form-check-inline">
								...
								name="sort" 
								value="lowprice"
								${param.sort or param.sort eq 'lowprice'? 'checked' : '' }"
								onchange="changeSort()"/>
							...
							<div class="form-check form-check-inline">
								...
								name="sort" 
								value="highprice"
								${param.sort or param.sort eq 'highprice'? 'checked' : '' }
								onchange="changeSort()"/>
					...
					<div class="col-4">
						<div class="btn btn-outline-secondary btn-sm" onclick="removeCheckedProducts()">선택삭제 </div>
					</div>
				</form>				
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		/*
			document.getElementById("form-products")는  <form>를 표현하는 엘리먼트 객체를 찾아서 반환한다.
		...
		*/
		...
		function removeCheckedProducts() {
			/*
				...
				document.querySelector("input[type=checkbox]");  input[] 을 input [] 이렇게 띄어쓰면 자식? 에서 찾게된다. 
					- 태그명이 input이고, type속성값이 checkbox인 모든 엘리먼트를 선택한다.
				document.querySelector("input[type=checkbox][name=no]");
					- 태그명이 input이고, type속성값이 checkbox이고, name속성값이 no인 모든 엘리먼트를 선택한다.
				document.querySelector("input[type=checkbox][name=no]:checked");
			...
			*/
			...
			if (checkedCheckboxes.length == 0) {
				alret("체크된 체크박스가 없습니다.");
				return ; 
			}
			
			let form = document.getElementById("form-products");
			//<form> 엘리먼트 action 속성값을 delete로 변경한다. 
			//<form>을 제출하면 localhost/product/delete 요청을 서버로 보내게 된다. 
